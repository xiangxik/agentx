...
public class OpenAiCompatibleChatGateway {
  ...
  public OpenAiCompatibleChatGateway(ObjectMapper objectMapper) {
    ...
    try {
      ...
      if (response.statusCode() >= 400) {
        throw new IllegalStateException(
            "OPENAI_COMPATIBLE_HTTP_" + response.statusCode() + ": " + response.body());
      }

      JsonNode root = objectMapper.readTree(response.body());
      JsonNode contentNode = root.path("choices").path(0).path("message").path("content");
      String content = contentNode.isMissingNode() ? "" : contentNode.textValue();
      if (content == null || content.isBlank()) {
        throw new IllegalStateException("OPENAI_COMPATIBLE_EMPTY_CONTENT");
      }
      return new GatewayResponse(content.trim());
    } catch (IOException exception) {
      throw new IllegalStateException("OPENAI_COMPATIBLE_IO_ERROR", exception);
    } catch (InterruptedException exception) {
      Thread.currentThread().interrupt();
      throw new IllegalStateException("OPENAI_COMPATIBLE_INTERRUPTED", exception);
    } catch (JacksonException exception) {
      throw new IllegalStateException("OPENAI_COMPATIBLE_PARSE_ERROR", exception);
    }
  }
  ...
  public record ChatRequest(String apiEndpoint, String apiKey, String modelCode, String question, String knowledgeContext) {}
